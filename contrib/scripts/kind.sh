...
secondary_interface=false
if [ "${1:-}" = "--secondary-interface" ]; then
  secondary_interface=true
  shift
fi
readonly secondary_interface
...
usage() {
  echo "Usage: ${PROG} [--xdp] [--secondary-interface] [control-plane node count] [worker node count] [cluster-name] [node image] [kube-proxy mode] [ip-family]"
}
...
if [ "${secondary_interface}" = true ]; then
